...
sWorkJobResult WorkEscort(sGirl& girl, bool Day0Night1, cRng& rng)
{
    ...
    {
        ss << "${name}has some particularly aggressive tendencies that make her a poor choice for patrons seeking polite conversation. Even if she carries a conversation well, the undercurrent of restrained hostility will detract from her performance.\n";
        escort -= 2;
    }
    if (girl.has_active_trait("Bimbo"))
    {
        ss << "${name} is a complete ditz. Her repertoire of conversation is limited to her favorite dresses and her many powerful feelings on makeup brands. This will stymy her abilities as an escort, no matter how well she does otherwise.\n";
        escort -= 1;
    }
    if (girl.has_active_trait("Nervous") || girl.has_active_trait("Shy"))
    {
        ss << "${name} is very shy with new people. For a job that requires meeting with new people constantly, this is a bad trait to possess. Her nervousness will translate into some awkward conversational moments, no matter how great her charisma is.\n";
        escort -= 2;
    }
    if (girl.has_active_trait("Retarded"))
    {
        ss << "${name} is retarded, and this is a natural obstacle to most conversations. For those men that do not want a prolonged conversation on her favorite colors or a description of her last \"poopy,\" which is the vast majority of men, she may have some trouble as an escort.\n";
        escort -= 3;
    }
    //good
    if (girl.has_active_trait("Psychic"))
    {
        ss << "${name} has some psychic abilities, meaning she has a natural advantage in conversation. She knows what the client likes and can avoid the subjects that he dislikes. This may come in very handy.\n";
        escort += 2;
    }
    if (girl.has_active_trait("Charismatic") || girl.has_active_trait("Charming"))
    {
        ss << "${name} is very personable and charming. Conversations with her will be easy, and this gives her an advantage as an escort.\n";
        escort += 2;
    }
...
case Es_Regular:
{
    ...
    ss << "${name} has been requested by a regular client, a man who has called on her before and now is willing to pay extra for a special assignation. His new boss has asked the client out for a double date with the client's girlfriend and his own wife so he can get to know his new employee better. The problem is that the client does not have a girlfriend. ${name} is to arrive at a local restaurant, equipped with a suitable dress and pair of shoes, ready to chat amiably. The client sends a list of written details about their \"shared relationship\" for her to memorize.\n";

    //how prepared and when does she arrive...
    if (prepare > 99)//very early very prepared
    {
        ss << "${name} immediately delves into the customer biography on the client and does some independent research on him. She memorizes the list in total detail, and even scopes out the places where they are supposed to have met and had their previous dates. She arrives at the door of the restaurant at the exact appointed time.\n";
        escort += 3;
    }
    else if (prepare >= 75)//early and prepared
    {
        ss << "${name} arrives at the door of the bustling downtown restaurant just in time. She actually arrived half an hour ago, and has spent her time scoping out the restaurant and its patrons and contemplating conversation topics. She read the biography that your escort clients are obligated to provide, and knows enough details about her repeat client and their fictional relationship that she is ready to make it appear believable.\n";
        escort += 2;
    }
    else if (prepare >= 50)//on time and prepared
    {
        ss << "${name} arrives just on time, with a relatively good understanding of the details of their shared \"relationship\" so she can appear as if she really is his girlfriend.\n";
        escort += 1;
    }
    else if (prepare >= 25)//kind prepard sort late
    {
        ss << "${name} arrives ten minutes late, dressed suitably and mulling over conversation topics. She knows her lateness will probably upset him, but she has at least reviewed some of the list of details of their \"relationship\" so that she does not need to completely improvise.\n";
    }
    else//very late and uprepared
    {
        ss << "${name} arrives at the restaurant half an hour late, barely groomed and with minimal efforts to make herself presentable. She has not memorized anything on the list and will just have to improvise the details of their \"relationship\".\n";
        escort -= 1;
    }

    //beauty check
    ss << "${name} enters the restaurant and locates the client, his boss, and his boss's wife sitting at a table for four in a quiet corner. She approaches the table, ready to make her first impression.\n";
    if (girl.beauty() > 99)
    {
        ss << "${name} is not merely beautiful, she is absolutely stunning. The client knows this already, and while he is still somewhat transfixed by her charms, he is able to focus on his boss, who is practically drooling himself. The wife moves uncomfortably in her seat as this moment draws out longer. \"My goodness,\" says the boss to the client. \"You certainly did well for yourself!\" What only ${name} can see during their little exchange, however, is that the wife continues shifting in her chair uncomfortably, staring deep into her eyes while casually rubbing her lips with one finger. Interesting.\n";
        escort += 3;
    }
    else if (girl.beauty() >= 75)
    {
        ss << "${name} is a very attractive woman, and while the boss's wife is a little threatened and perturbed by how long her husband stares at his employee's \"girlfriend,\" the client is pleased by the wink of congratulations that his boss shares with him a moment later. What neither the boss nor the client see, however, is the long interested stare that the wife gives her. There is an interest in that glance that goes beyond the professional kind.\n";
        escort += 2;
    }
    else if (girl.beauty() >= 50)
    {
        ss << "${name} is attractive, but not enough so to be threatening to the boss's wife, and while the boss gives her an appraising and thorough review with his eyes, his wife simply smiles pleasantly at her and asks that she tell her all about where she gets her makeup.\n";
        escort += 1;
    }
    else if (girl.beauty() >= 25)
    {
        ss << "While ${name} is not blessed with a lot of natural beauty, this certainly does no harm here. The client has already met her and been impressed by her other attributes, and while his boss does not pay much attention to her, his wife seems happy that she is the most attractive woman at the table.\n";
    }
    else
    {
        ss << "While the client knows ${name} and is aware of her plainness, the boss and his wife have not seen her before, and she forces herself to swallow her pride when the wife immediately makes a sarcastic comment about how pleased she is that her husband continues to employ individuals with such \"high standards\".\n";
        escort -= 1;
    }
    ...
    if ((girl.has_active_trait("Massive Melons") || girl.has_active_trait("Abnormally Large Boobs")
        ...
        || girl.has_active_trait("Busty Boobs") || girl.has_active_trait("Giant Juggs")) && rng.percent(75))
                {
                    ss << "${name} knows that this assignation is all about pleasing the boss. So when she sees his eyes lock onto her substantial tits, she gives a quick victorious smile to her \"boyfriend.\" The boss's wife glances down at her own modest breasts with discomfort, but soon breaks the silence with a comment about the lovely weather.\n";
                    escort += 1;
                    if (girl.has_active_trait("Exhibitionist") && rng.percent(50))
                        {
                            ss << "Being a bit of an exhibitionist, ${name} decides to give the boss a quick \"accidental\" show when his wife is distracted by the menu. She leans forward enticingly to study the menu, giving him a full view of her deep cleavage.\n";
                            if (rng.percent(75))
                            {
                            ...
                            {
                                ss << "The wife turns back just in time to find her husband staring shamelessly into that cleavage, but while ${name} expects everything to get a little rocky after that, the wife only turns to her with a knowing smile and a quick appreciative stare at her cleavage as well.\n";
                                escort += 2;
                            }
                        }
                }
    ...
    {
        ss << "The boss immediately takes note of ${name}'s sexy body with his eyes, and swallows hard, unable to form words for a few seconds. His wife does not seem to notice the effects on her husband as she is also staring, trying to figure out what it is that is so transfixing about her.\n";
        escort += 1;
    }
    ...
    {
        ss << "The dress and makeup are not enough to hide the bruises covering ${name}'s body, and both the boss and his wife share a significant look before shifting their gaze to the client, clearly expecting an answer. \"I fell down some stairs,\" offers ${name} with embarrassment, though they do not seem fully convinced of her explanation.\n";
        escort -= 1;
    }
    ...
    ss << "The four order a few bottles of sake to share amongst themselves and a veritable pile of sushi. For once in her work as an Escort, the focus of the attention tonight is not on ${name} but on her client, and her only role is to appear supportive of her \"boyfriend\" in front of her boss, and to make their relationship believable.\n";
    if (girl.service() > 99)
    {
        ss << "${name} knows everything about this fictitious relationship. She also knows about the details of the job that the client just got, and is able to talk with great specificity about the circumstances of their meeting, where they enjoy going on dates, and how proud she was when he came home�here, with perfect details of his house�to tell her about his new job.\n";
        escort += 3;
    }
    else if (girl.service() >= 75)
    {
        ss << "Given the amount of research that ${name} conducted prior to arriving, the relationship is easily believable. She knows everything about the two of them, and is able to describe with great accuracy how they met, and all of their previous dates. When the conversation drifts towards her job, she is able to provide substantial details about her fictitious employment.\n";
        escort += 2;
    }
    ...
    //random stuff
    if (girl.has_active_trait("Dominatrix"))
    {
        ss << "This conversation would have gone better in one respect: if ${name} were not such a dominant girl, she would have wisely allowed the client to lead the conversation, as it is ultimately his relationship with his own boss that is ultimately most important. Her constant interjections and attempts to steer the conversation where she wants it to go did give the impression that the client had picked a particularly chatty girl with some difficulty at understanding social cues.\n";
        escort -= 1;
    }

    if (girl.has_active_trait("Alcoholic"))
    {
        ss << "The conversation would have gone better if ${name} had not started shooting the sake as fast as it was delivered. Her alcoholism managed to capture everyone's attention, and the boss and his wife did share one silent concerned look with the client as ${name} started her fifth bottle.\n";
        escort -= 1;
    }
    else if (girl.has_active_trait("Social Drinker"))
    {
        ss << "The sake flowed freely throughout the dinner, and ${name}, being a social drinker, energetically convinces all three others to keep pace with her. They all start enjoying themselves with the social lubricant in no time, and the dinner party took a more jovial tone than any of them had expected.\n";
        escort += 1;
    }

    if (girl.has_active_trait("Porn Star"))
    {
        ss << "Throughout the dinner, ${name} did notice the boss occasionally staring at her with curiosity. \"I could swear I've seen you before somewhere,\" he muttered, unable to put his finger on it, but somehow all the more interested in her. ${name} suspects that her work as a porn star may mean that he certainly has seen her before, and he has probably seen a lot of other people putting their fingers, and other body parts on \"it,\" too.\n";
        escort += 1;
    }

    if (girl.has_active_trait("Bisexual"))
    {
        ss << "${name} is finely attuned to the sexual signals of both men and women, since she is attracted to both, and she can easily detect the vibes that the boss's wife is giving. They are both looking her over with more than casual interest, and this gives ${name} some interesting ideas.\n";
        escort += 2;
    }
    ...
    if (escort >= 9)
    {
        ss << "The four of them are laughing like old friends by the time the bill arrives, and ${name} can feel looks of attraction and lust coming her way from the client, which is expected, and from the boss, which is a nice bonus. She can also feel them coming from the boss's wife, which was something she had not expected this evening, but the looks were strong enough that she was not completely surprised when the wife looked knowingly at her husband and whispered something in his ear. He smiles for a second and then looks at his new employee and ${name}. \"I know this may be a bit unorthodox,\" he begins, \"but we've been having a great evening, and we wanted to know if.. maybe.. the two of you wanted to keep the night going at our place?\" The client looks at ${name} with surprise and amazement, but then thinks about it and smiles. His look changes to that of an open question, meaning that he will take ${name}'s lead on the answer.\n";
        fame += 2;
        group_offer = true;
        cust_wealth = 2;
    }
    ...
    {
        ss << "As the dinner comes to a close, the client fidgets and looks for excuses for the two of them to leave. As soon as one appears, he escorts ${name} quickly out of the restaurant. \"I can't believe that worked,\" he says, nervously, \"but it almost didn't. Thanks, but let's just call it a night. Take your fee,\" he whispers, discreetly handing her an envelope with her payment. \"I'll tell them you had a work thing tonight and go chat some more. Thanks,\" he says dismissively as he turns back towards the restaurant.";
    }
    else
    {
        ss << "The dinner ends with the boss and his wife sharing a skeptical glance as the client hurries ${name} out the door to an imaginary late-night work assignment. \"That was absolutely terrible,\" he whispers ferociously to her when they are alone outside. \"Now I have to go back in and do damage control. For all I know, you may have just cost me my job. Maybe that's my fault for taking this risk with you, but I can definitely tell you one thing: I will not be paying for this. Tell your boss that he needs to be much more careful with his assignments in the future.\" He turns angrily on his heels, composes himself, and reenters the restaurant, leaving ${name} to walk back to your establishment, asking herself about the myriad things she did wrong.";
        fame -= 1;
        cust_wealth = 0;
    }
    ...
    if (group_offer)
    {
        if (girl.morality() >= 50)
        {
            ss << "${name} is savvy enough to know what they are suggesting, but is too conservative in her morals to consider having sex with all of them. She politely declines, much to the client's regret, but tells them that she did have a wonderful evening. The client escorts her outside and thanks her for her incredible performance. \"I know that the group offer might not be your thing,\" he says, \"but I thought maybe you and I might keep our little fake relationship going for another hour or two, if you're interested,\" he concludes, waiting hopefully for her answer.\n";
            sex_offer = true;
        }
        else if (girl.libido() <= 50)
        {
            ss << "${name} politely considers the offer, but the fact of the matter is that she has no interest in sex with anyone tonight. She invents an excuse about having a work event to get to, apologizing profusely, but suggesting that maybe the four of them grab another dinner sometime soon. The client is surprised at the suggestion, since it means that he may have to pay for another group date with his \"girlfriend,\" but he is hardly angry, as the possibility of fucking his boss's wife is a bit too enticing for him to ignore. The client escorts her outside and discreetly hands her an envelope with a substantial tip for her excellent work tonight.";
        }
        else
        {
            if (is_virgin(girl))
            {
                ss << "${name} smiles at the offer, and leans a bit over the table before answering. \"I think we would love to join you,\" she says. They all grin with anticipation, getting up to leave. As they walk out the door, ${name} leaves the client's side for a moment to speak to the wife. \"I think you should know,\" she whispers, \"that I am still a virgin, and that the two of us have decided to wait until marriage.\" The wife appears disappointed, but livens as ${name} continues: \"This doesn't mean that the two of us can't get to know each other a little better at your place, though. As long as the boys know that my body is off limits, I think we could still have a very enjoyable evening.\" The wife smiles and turns to whisper something to her husband while ${name} discreetly informs the client of the arrangement. He smiles as well, and the four of them soon arrive at a lovely expensive home, where they soon begin undressing themselves around a large bed.\n";
                if (girl.lesbian() > 99)
                {
                    ss << "Within minutes, the wife has her lips wrapped around her husband's cock as the client spears her ass. She writhes in pleasure, pulling ${name}'s head towards her open pussy. ${name} licks with masterful skill, nibbling with expertise on the clitoris as she thrusts her fingers into the pussy, timing each thrust with the attacks on her ass and mouth. She is truly a master at cunnilingus, and by the time the men have exploded their semen into the sweaty woman, she is screaming with passion on her fifth powerful orgasm, spraying her juices everywhere. They all lie panting on the bed afterwards, with the wife purring in ecstasy.\n";
                    fame += 3;
                }
                else if (girl.lesbian() >= 75)
                {
                    ss << "Within minutes, the wife has her lips wrapped around her husband's cock as the client spears her ass. She writhes in pleasure, pulling ${name}'s head towards her open pussy. ${name} passionately attacks her clitoris between the thrusts, fingering her pussy vigorously. The four fuck in ecstasy for half an hour, with the wife rocking back and forth with three powerful orgasms, spurting her juices all over ${name}'s face just as the client explodes in her ass and she swallows deeply from her husband's cum. The four lay there, panting, on the bed.\n";
                    fame += 2;
                }
                else if (girl.lesbian() >= 50)
                {
                    ss << "Within minutes, the wife has her lips wrapped around her husband's cock as the client spears her ass. She writhes in pleasure, pulling ${name}'s head towards her open pussy. ${name} licks with skill, timing her tongue to hit with the client's thrusts into her ass. She inserts a few fingers as she nibbles on the clitoris, and by the time the wife has collapsed onto the bed, leaking cum from her ass and mouth, she has enjoyed two powerful orgasms.\n";
                    fame += 1;
                }
                else if (girl.lesbian() >= 25)
                {
                    ss << "Within minutes, the wife has her lips wrapped around her husband's cock as the client spears her ass. She writhes in pleasure, pulling ${name}'s head towards her open pussy. ${name} licks with the limited skill she possesses, being somewhat new to the act of pleasuring another woman. Fortunately, with the help of the two men, ${name} is able to bring one good orgasm out of her before the wife collapses on the bed, covered in their cum.\n";
                }
                else
                {
                    ss << "Within minutes, the wife has her lips wrapped around her husband's cock as the client spears her ass. She writhes in pleasure, pulling ${name}'s head towards her open pussy. ${name} licks it amateurishly, completely ignorant of how to satisfy another woman. While the powerful fucking continues, the three of them pay little attention to the inexperienced efforts of ${name}, and mostly manage to please each other. They are all somewhat surprised by her lack of skills, but by the time the wife manages to bring both men to orgasm, leaking cum from her mouth and ass, there is no doubt that they still had a good time, despite her.\n";
                    fame -= 1;
                }
            ...
            {
                ss << "\"What a wonderful idea,\" announces ${name}, bringing anticipatory grins from the other three. \"I'm sure we would love to share ourselves with you this evening.\" The four of them are soon entering the door of the boss's lovely expensive home, and within minutes they are all undressing each other on a large bed. \"The best business arrangements are about sharing, after all,\" smiles ${name} as she positions herself to pleasure all three of them.\n";
                if (girl.group() > 99)
                {
                    ss << "While some women might be at a loss as to how to pleasure two cocks and a pussy at the same time, ${name} is a master at group fucking and soon shows her expertise. She rotates herself constantly between the other three, sometimes just sucking them alternately with her mouth, sometimes enjoying a cock in her mouth and another in her ass as the wife fists her with reckless abandon. Both women have orgasmed at least a handful of times by the time the men pull out of them, push their heads together, and shoot their loads violently into ${name}'s mouth. She swallows with a wan smile, giving the wife's asshole onoe last vigorous fingering. The four collapse in an ecstatic pile on the bed, with all three of the others staring with incredulity at ${name}. \"You are incredible,\" they agree, panting and gasping for air.\n";
                    fame += 3;
                }
                else if (girl.group() >= 75)
                {
                    ss << "${name} definitely knows her way around a group fuck, and soon she is rotating the two men between her ass and pussy as she furiously nibbles on the wife's clit. They roll around each other, moaning with pleasure, as ${name} brings orgasm after orgasm from the other woman, whilst simultaneously bouncing with enthusiasm on the two cocks buried inside her. Soon ${name} is leaking with cum from her lower holes and streaked with pussy juices on her mouth. The three others collapse in ecstasy around her, amazed by her incredible performance. \"What a night,\" says the boss, gasping for air on his corner of the bed.\n";
                    fame += 2;
                }
                else if (girl.group() >= 50)
                {
                    ss << "${name} is not a stranger to group sex, and she is soon enjoying the boss's cock in her ass and the client's in her pussy as she licks voraciously at the wife's clit. The four of them rotate between each other for a blissful half hour before the men pull out, just in time for both women to kiss and stroke each other passionately as the men shoot their loads all over their sensuously mashing tits. The four collapse happily into the bed, panting with ecstasy.\n";
                    fame += 1;
                }
                else if (girl.group() >= 25)
                {
                    ss << "${name} has only limited experience in these kinds of situations, however, and her efforts at fucking the cock in her ass while licking the wife's pussy are merely amateurish. Nevertheless, the four of them are able to enjoy the time together, and as the client explodes his cum deep into the wife's ass, ${name} feels a spray of the wife's pussy juices in her mouth as the boss shoots his load deep into her womb. The four collapse contended into the bed.\n";
                }
                else
                {
                    ss << "Unfortunately, ${name} has no idea what to do with two cocks and a pussy. She tries to lick the pussy, but is soon distracted by the cock filling her ass and stops licking while she concentrates on her own sensations. The wife is surprised and disappointed, as are the other men, who eventually turn their attentions to the wife when they realize that anything more than one partner confuses ${name} into inaction. By the end of the session, the two men are double-teaming the wife, and ${name} is relegated to occasional licks. She watches, embarrassed in her own lack of skills, as they poor their cum all over the rabidly panting woman.\n";
                    fame -= 1;
                }
            ...
            if (girl.oralsex() > 99)
                    {
                        ...
                        {
                            ss << "\"I know you'll enjoy this,\" says ${name} as she drops to her knees, pulls out his cock from his pants, and puts her masterful oral skills to work. She cannot take him deeply out of concern for her gag reflex, but she knows that her incredible mouth will be more than enough to pleasure him anyway, and licks and sucks various parts of his cock and balls as he builds towards orgasm, moaning with pleasure the whole time. She forces herself to take him deeply right before he cums, making herself gag on him but using her involuntary reflex as another way of massaging his shaft. He shoots a violent explosion of cum into her mouth and then staggers back, collapsing on his couch. \"That was amazing,\" he pants as ${name} licks the cum from her lips.\n";
                            fame += 2;
                        }
                        else
                        {
                            ss << "\"I know you'll enjoy this,\" says ${name} as she drops to her knees, pulls out his cock from his pants, and puts her masterful oral skills to work. She takes him deeply in one swallow and holds him there, locked in her as she swallows repeatedly to massage him with her throat. She stays there, intentionally depriving herself of oxygen for his pleasure, until she feels him building towards orgasm. She then releases, teasing him. \"Not yet,\" she whispers before turning her phenomenal tongue to his balls. She returns to his shaft as he recovers himself, and then slowly and masterfully builds the tension until she knows the time is approaching. She then plunges herself deep onto his cock again, licking his balls as her throat plays around his tip, until he shoots a violent explosion of cum straight down to her stomach. \"That was absolutely incredible,\" moans the client as he collapses onto his couch.\n";
                            fame += 3;
                        }
                    }
                    ...
                    else if (girl.oralsex() >= 50)
                    {
                        if (girl.has_active_trait("Strong Gag Reflex") || girl.has_active_trait("Gag Reflex"))
                        {
                            ss << "\"Allow me to show my appreciation for a lovely evening,\" says ${name} as she falls to her knees and pulls his cock out from his pants. She lovingly licks every part of his crotch, teasing his balls with her tongue and then engulfing his tip with her mouth as she rotates her tongue around the head. She avoids going any deeper out of concern for her gag reflex, which ultimately results in a mediocre blowjob, but the client is soon shooting semen until it leaks out of the sides of her mouth.\n";
                        }
                        else
                        {
                            ss << "\"Allow me to thank you,\" says ${name} as she lowers herself to her knees and removes his cock from his pants. She takes his erection deep into her mouth and massages it with her throat, and then begins to pump her head back and forth, stopping to occasionally lick his balls. It is not the best blowjob he has ever received, but it is nevertheless very pleasurable, and ${name} is soon rewarded for her efforts with an eruption of cum into her mouth.\n";
                            fame += 1;
                        }
                    }
                    else if (girl.oralsex() >= 25)
                    {
                        if (girl.has_active_trait("Strong Gag Reflex") || girl.has_active_trait("Gag Reflex"))
                        {
                            ss << "\"Maybe this will help,\" she says while lowering herself to her knees and pulling his cock free from his pants. She licks the tip seductively and then engulfs the head with her mouth, sucking gently. The client groans with pleasure, and ${name} keeps sucking the tip of his cock. And keeps sucking. For minutes. She does not vary her technique, or move to lick or suck anything else, or take the cock deeper into her mouth. She simply sucks as if it is a nipple and she is waiting for milk to come out, which the client supposes is not too far from the truth. Still, it is not a great blowjob, and it is not until he takes over and starts thrusting himself into her that he is able to build towards orgasm. ${name}'s gag reflex suddenly kicks in, though, which is probably why she was staying so shallow on the tip in the first place, and she is soon coughing all over his cock and pulling herself away to recover. This kills the mood for a period, and it is not until she regains composure and the client masturbates himself into her mouth that he is able to cum. The whole experience leaves him slightly disappointed.\n";
                            fame -= 1;
                        }
                        else
                        {
                    ...
                    ss << "${name} licks the remaining cum from her lips with a smile. \"If you happen to have a real girlfriend, I hope she can do that as well as I can,\" she flirts as she grabs her clothing and prepares to go. \"And if you ever need a fake girlfriend again for a night, give me a call.\"";
                    imagetype = IMGTYPE_ORAL;
                    girl.oralsex(2);
                    girl.upd_temp_stat(STAT_LIBIDO, -20, true);
                }

        if (roll_sex >= 50 && !is_virgin(girl))/*sex*/
        {
            if (girl.normalsex() > 99)
        ...
        else if (roll_sex >= 40 || anal)/*Anal*/
                {
                    ...
                    {
                        ss << "${name} turns around, lifting her dress and giving him a good view of her ass. For the first time, the client is able to see a faint outline of plastic against her panties. \"I bet you didn't know I had this in me the whole time,\" ${name} giggles as she deftly moves the crotch of her panties to the side, revealing the base of a buttplug sticking out of her ass. \"I'm nice and ready for you to take its place,\" she purrs, pulling out the plug and presenting her gaping ass to him.\n";
                        fame += 1;
                    }
                    else if (girl.has_item("Large Buttplug"))
                    {
                        ss << "${name} turns around, lifting her dress and giving him a good view of her ass. For the first time, the client is able to see a faint outline of plastic against her panties. \"I bet you didn't know I had this in me the whole time,\" ${name} giggles as she deftly moves the crotch of her panties to the side, revealing the base of a buttplug sticking out of her ass. \"I'm nice and ready for you to take its place,\" she purrs, tugging with her hand at the base of the buttplug. As she pulls it out, her ass clutches the plug and expands dramatically. The client is amazed at how far it keeps stretching, until the full massive plug plops down onto the floor, leaving her ass wide open and gaping in front of him. He pauses in wonder, having never realized that a girl's ass could carry anything so massive.";
                        fame += 2;
                    }
                    if (girl.anal() > 99)
                    {
                        ss << "Once ${name} has turned around, lifted her skirt, and displayed her ass to the client, he grins devilishly, spits on his hand, lubricates his cock, and then plunges into her. ${name} sighs with immense pleasure, truly happy to have her ass filled by his cock. She holds him back with her hand, asking that she be able to do the work for him. Soon she is gently massaging his dick with perfectly-timed squeezes of her asshole, purring with rapture. She then begins pumping herself back and forth on him, forcing him deep inside her, picking up the pace to a furious level and then slowing it down to keep him from cumming too early. After twenty heavenly minutes inside her, she begins throwing her hips backwards and forwards, riding his cock like the anal master that she is. He screams with bliss as he erupts violently into her ass, shooting string after string of cum deep inside her. \"Incredible!\" he exclaims, collapsing panting onto his couch.\n";
                        fame += 3;
                    }
                    else if (girl.anal() >= 75)
                    {
                        ss << "Once ${name} has turned around, lifted her skirt, and displayed her ass to the client, he grins devilishly, spits on his hand, lubricates his cock, and then plunges into her. ${name} moans with pleasure and begins to pump herself expertly around his manhood. \"Fuck my ass,\" she purrs, massaging him with well-timed squeezes as she forces him deeper into her with increasing thrusts. She gyrates herself expertly around him, and within a matter of minutes, the client is sweating and panting towards climax. \"Cum in me!\" she cries, as he fills her bowels with a violent eruption of cum.\n";
                        fame += 2;
                    }
                    else if (girl.anal() >= 50)
                    {
                        ss << "Once ${name} has turned around, lifted her skirt, and displayed her ass to the client, he grins devilishly, spits on his hand, lubricates his cock, and then plunges into her. ${name} moans with pleasure at the invasion and begins gyrating her hips around his cock, forcing him deeper into her and enticing him with the vision of her ass bouncing against his thighs. She does not know how to squeeze her ass at the right moments, but with her efforts and the mere fact that his cock in her ass is plenty of encouragement enough, soon he is shooting cum deep inside her bowels. \"Whoa,\" he pants, collapsing on the couch afterwards.\n";
                        fame += 1;
                    }
                    else if (girl.anal() >= 25)
                    {
                        ss << "Once ${name} has turned around, lifted her skirt, and displayed her ass to the client, he grins devilishly, spits on his hand, lubricates his cock, and then plunges into her. ${name} moans with a combination of pleasure and pain and tries to push herself back and forth to stimulate his cock. Her motions are amateurish and not very pleasurable, but the client can hardly complain, and soon he is shooting his cum deep inside her bowels.\n";
                    }
                    else
                    {
                        ss << "Once ${name} has turned around, lifted her skirt, and displayed her ass to the client, he grins devilishly, spits on his hand, lubricates his cock, and then plunges into her. ${name} whimpers at the sudden intrusion, and then proceeds to lie there motionlessly while he rams himself back and forth. As he builds to climax, though, he realizes that the sounds she is making are not moans of pleasure but grunts of pain, and he sees that she is crying with discomfort. He forces himself to slow his pace, afraid to hurt her further, and then pulls out so only the tip is still inside her as he wraps his own hand around his cock and strokes himself to climax. He is surprised that ${name}, who seemed to so eagerly want a cock in her ass, was so bad at taking one, and is by and large disappointed with the entire experience.\n";
                        fame -= 1;
                    }
                    ss << "${name} lies beside the client for a moment, stroking his chest with her fingers as the cum leaks from her ass. \"That was amazing,\" she purrs, before she stands to gather her clothes to leave. \"If you have a real girlfriend, I hope she lets you do that to her ass from time to time,\" she flirts. \"But if you need a fake girlfriend again some night, you know where to find me.\"";
        ...
        else if (roll_sex >= 25)/*titty job*/
                {
                    ...
                    {
                        ss << "While ${name} is enthusiastic, she has some difficulty using her breasts to bring him off. Her lack of skills result in her spending more time smacking his dick with her nipples than building up a good pace. He is not very upset�few men in his position would be�but he does need to use his own hands a little to build up enough tension that he can cum.\n";
                    }
                    else
                    {
                        ss << "After a moment, though, the client realizes that ${name} really has no idea how to titty-fuck, and her efforts are lackluster at best. In the end, he has to use his imagination and help with his own hands in order to cum.\n";
                        fame -= 1;
                    }
                    ss << "After he erupts cum all over her face, ${name} smiles, cleans herself off, and turns to collect her things to go. \"I'm afraid that's all our relationship can enjoy for tonight,\" she continues. \"Though if you ever need me to be your girlfriend again for the night, please give me a call.\"";
        ...
        else /*Hand job*/
                {
                    ...
                    ss << "${name} sensuously licks the semen from her fingers while the client recovers himself. \"If you have a real girlfriend, I hope she can do that as well as your pretend one,\" she smiles, packing her things to leave. \"And if you ever need a fake girlfriend again for a night, give me a call.\"";
...
case Es_Commoner:
{
    ...
    //COMMENER
    //telling what she is meeting and where
    ss << "${name} has been requested by a commoner client, an artisan who has used your services before and is looking for a refined and charming young lady to join him for dinner at a popular restaurant in the Crossgate city center. She is to wear casual clothes and meet him for drinks, dinner, and conversation.\n";
    ...
    {
        ss << "${name} arrives just on time, dressed in attractive casual clothes, and made up to entice him with her charms. She has prepared a few conversation topics to help the date along.\n";
        escort += 1;
    }
    else if (prepare >= 25)//kind prepard sort late
    {
        ss << "${name} arrives ten minutes late, dressed in suitable casual clothes and mulling over conversation topics. She knows her lateness will probably upset him, but hopes that she can improve the date with witty banter.\n";
    }
    else//very late and uprepared
    {
        ss << "${name} arrives at the restaurant half an hour late, barely groomed and with minimal efforts to make herself presentable. She does not even think about whether this will upset the client.\n";
        escort -= 1;
    }

    //beauty check
    ss << "${name} enters the restaurant and locates the artisan, sitting at a table for two in a quiet candlelit corner.\n";
    if (girl.beauty() > 99)
    {
        ss << "He looks up as she approaches, and then he just stares in silence for an awkward silence. \"I'm sorry,\" he stammers, collecting himself. \"You are radiantly beautiful, and I lost my words for a moment.\" ${name} smiles politely in response and makes a kind remark in return as he stands, pulls out her chair, and informs her that he has ordered a bottle of wine for them to share while they consider their food choices.\n";
        escort += 3;
    }
    else if (girl.beauty() >= 75)
    {
        ss << "He looks up as she approaches and smiles radiantly. \"You are absolutely lovely, ${name}. I'm so glad you could join me this evening.\" He stands and pulls out the seat for her, and she can feel his eyes quickly scanning her backside. ${name} smiles politely and sits.\n";
        escort += 2;
    }
    else if (girl.beauty() >= 50)
    {
        ss << "He looks up as she approaches and smiles. ${name} is not a bombshell looker, but she can hold her own, and she smiles back, allowing him to take her all in. \"I'm so glad you could join me,\" he begins, his eyes returning to her face. \"Please have a seat.  I've ordered us a bottle of wine, and I hope you'll like it.\"\n";
        escort += 1;
    }
    else if (girl.beauty() >= 25)
    {
    ...
    {
        ss << "He looks up as she approaches, and tries but fails to disguise his disappointment at her appearance. ${name} is not an attractive girl, but she does not like to be reminded of this. She swallows her pride and sits down, hoping that her conversational abilities will redeem her.\n";
        escort -= 1;
    }
    ...
    if (girl.has_active_trait("Massive Melons") || girl.has_active_trait("Abnormally Large Boobs")
        ...
        || girl.has_active_trait("Busty Boobs") || girl.has_active_trait("Giant Juggs"))
                {
                    ss << "${name} catches the client's eyes going briefly to her substantial breasts,";
                    titty_lover = true;
                    if (rng.percent(75))
                    {
                    ...
                    {
                        ss << " which she is very used to experiencing. She is not so used to his reaction, though, which seems a bit disappointed. He mutters something about top-heavy ships being poorly made before he catches himself, meets her gaze, and smiles. It seems that ${name}'s great tits are not such an asset tonight.\n";
                        escort -= 1;
                    }
    ...
    else
    {
        if (rng.percent(40))
        {
            ss << "${name}  notices when the client flashes his eyes quickly over her breasts. She has always been a bit self-conscious about their small size, but she is surprised to see him smile back at her with greater appreciation. \"Aren't you just perfect,\" he smiles.\n";
            escort += 1;
            titty_lover = true;
            if (girl.has_active_trait("Exhibitionist") && rng.percent(50))
                {
                    ss << "As a bit of an exhibitionist and pleased at this unusual attention on her small breasts, ${name} decides to reward his interest by \"accidentally\" catching the top of her shirt on her bracelet, giving him a brief view of her pert tits and even a flash of nipple. \"Oh, how silly of me,\" she exclaims, readjusting herself.\n";
                    if (rng.percent(75))
                        {
        ...
        {
            ss << "${name} notices when the client flashes his eyes quickly over her small breasts, and she is mentally ready when she detects the disappointment in his eyes. He is a breast man, like so many, and ${name} is very familiar with the ways that the client tries to disguise his regret. She resolves to win him with her other charms.\n";
            escort -= 1;
        }
    }
    ...
    if ((girl.has_active_trait("Great Arse") || girl.has_active_trait("Deluxe Derriere") ||
            girl.has_active_trait("Phat Booty")) && !titty_lover)
    {
        ss << "${name} feels the client's gaze as she rests her fantastic ass in her chair. He is clearly mesmerized by it for a second, before he forces himself to stop staring and returns to his own chair.";
        escort += 1;
        ass_lover = true;
        if (girl.has_active_trait("Exhibitionist") && rng.percent(50))
                {
                    ss << "${name} does not mind his gaze, however, and her exhibitionist tendencies make her decide to keep his attention on her amazing ass for a few more moments. She sits so that her skirt hitches on the chair and rides up, giving him a brief view of the contours of her ass cheeks. \"Oh my,\" she giggles, quickly�but not too quickly�settling her skirt back to normal.\n";
                    if (rng.percent(75))
                        {
    ...
    if (girl.has_active_trait("Bruises"))
    {
        if (rng.percent(33))
        {
            ss << "The client cannot help but notice the bruises all over ${name}'s body, and his eyes flash momentarily with regret and sympathy for her life. He finds himself feeling inexplicably protective of her, and concerned for what may happen when she returns to her normal work.\n";
            escort += 1;
        }
        else if (rng.percent(33))
        {
            ss << "The client cannot help but notice the bruises all over ${name}'s body, and he finds them unseemly. He does not know what earned her these marks, but he clearly considers them to be inappropriate for a lady engaged in the escort profession.\n";
            escort -= 1;
        }
        else
        {
    ...
    {
        ss << "As the two of them finish their main course, the client pays the bill and rises to escort ${name} from her chair and walks with her outside the restaurant. \"Thank you for a nice evening,\" he says simply, handing her an envelope with her fee. \"I don't think the two of us are a good fit, but I still had a decent time tonight. Thank you, at least, for that,\" he concludes, turning to walk away. ${name} begins her trek back to your establishment, wondering what she could have done better.";
    }
    else
    {
    ...
    if (sex_offer)
    {
        if (girl.morality() >= 50)
        {
            ss << "${name} blinks in surprise at the offer. \"I'm afraid I just couldn't,\" she begins, doing her best to politely decline the offer. \"Thank you, but I'm a conservative kind of girl, and since we just met, I feel it would be wrong to do anything more.\" She blushes and makes ready to leave. \"Of course, I understand,\" stammers the client. \"A woman of your class and refinement would naturally be surprised by so forward an advance. A million apologies,\" he finishes, but ${name} brushes the apology away with tenderness, sparing his ego, as she turns to begin her trek back to your establishment.";
        }
        else if (girl.libido() <= 50)
        {
    ...
    if (sex_event)
    {
        if (titty_lover)
        {
            ...
            ss << "After he erupts cum all over her face, ${name} smiles, cleans herself off, and turns to collect her things to go. \"Thank you for that,\" she says as she approaches the door. \"You should know that, at the very least in this respect, you are better than any of the gentry I have known.\"";
            imagetype = IMGTYPE_TITTY;
            girl.tittysex(2);
            girl.upd_temp_stat(STAT_LIBIDO, -20, true);
        }
        ...
        {
            ss << "Once the two of them are behind closed doors, ${name} kisses him passionately and runs her hand to his crotch to feel his growing erection. \"Now how can I reward my perfect gentleman tonight?\" she asks, stripping off her casual clothes.";
        }

        //sex stuff needs linked up somehow
        if (!titty_lover && !ass_lover)
            {
                if (roll_sex >= 50 && !is_virgin(girl) || sex)/*sex*/
                {
                    if (girl.normalsex() > 99)
                    {
                        ss << "\"Let me do the work,\" she suggests, as she straddles him on the couch and envelops him with her hot, wet, pulsing pussy. She runs her fingernails gently over his chest as she circles herself on his cock. After a minute of heated teasing she begins to scratch him ever so slightly with her fingernails as she bounces up and down. She expertly matches the tempo to his thrusts, slowing when he gets close to orgasm, and picking it back up as he recovers. Then, when she feels he is ready, she deftly throws her left leg over his head and rotates so that now she is riding him with his back to her. She gyrates and humps like a wild woman, screaming in passion, until he erupts violently inside her. The two collapse in the couch together, sweating and panting, but very, very satisfied.\n";
                ...
                else if (roll_sex >= 40 || anal)/*Anal*/
                {
                    ...
                    ss << "${name} lies beside the client for a moment, stroking his chest with her fingers as the cum leaks from her ass. \"That was amazing,\" she purrs, before she stands to gather her clothes to leave. \"I'm glad we got to know each other a little better, and I hope you know that when it comes to tearing my ass open, you are better than any gentry I have ever met.\"";
                    imagetype = IMGTYPE_ANAL;
                    girl.anal(2);
                    girl.upd_temp_stat(STAT_LIBIDO, -20, true);
                }
                ...
                else/*oral*/
                {
                    ...
                    ss << "${name} licks the remaining cum from her lips with a smile. \"Thanks for the drink,\" she says, as she begins gathering her clothes to leave. \"I hope a gentleman like yourself is kind enough to call for me again sometime,\" she suggests as she heads for the door to return to your establishment.";
                    imagetype = IMGTYPE_ORAL;
                    girl.oralsex(2);
                    girl.upd_temp_stat(STAT_LIBIDO, -20, true);
                }
...
case Es_DeadBeat:
{
    ...
    //telling what she is meeting and where
    ss << "${name} has been requested by a client who has not used your services before, and has no reputation with your establishment. Because she is not famous enough to attract more reputable customers, she has no other clientele. She is to wear casual clothing and meet him at a nearby bar for a drink.\n";
    ...
    {
        ss << "${name} arrives at the bar half an hour late, barely groomed and with minimal efforts to make herself presentable. She does not even think about whether this will upset the client.\n";
        escort -= 1;
    }

    //beauty check
    ss << "${name} enters the bar and quickly locates the client.\n";
    if (girl.beauty() > 99)
    {
        ss << "He looks up as she approaches, and then he just stares in silence for an awkward silence. \"I'm sorry,\" he stammers, collecting himself. \"You are just the most beautiful woman I've ever seen. I don't mean that as some kind of lame line.. I mean it.\" ${name} smiles politely in response and makes a kind remark in return as he stands, pulls out her chair, and asks her if she would like to join him for a drink.\n";
        escort += 3;
    }
    else if (girl.beauty() >= 75)
    {
    ...
    {
        ss << "He looks up as she approaches and smiles. ${name} is not a bombshell looker, but she can hold her own, and she smiles back, allowing him to take her all in. \"I'm so glad you could join me,\" he begins, his eyes returning to her face. \"Please have a seat and I'll get you a drink.\"\n";
        escort += 1;
    }
    else if (girl.beauty() >= 25)
    {
    ...
    if (girl.charisma() > 99)
    {
        ss << "${name} and the client spend the next hour joking and bantering like old friends. She is wonderfully charismatic, and while she expertly keeps the conversation on comfortable subjects for him, she knows that he would be happy talking with her about anything. By the end of the hour he is wiping tears of laughter from his eyes and smiling at her with deep appreciation.\n";
        escort += 3;
        if (girl.has_active_trait("Alcoholic") && rng.percent(50))
        {
            ss << "${name}He also appreciates the sheer volume of alcohol that ${name} has consumed in the last hour. He accurately suspects that she may be an alcoholic, but she seems to be able to handle it well, and while this concerns him, it has hardly ruined his evening.\n";
            escort -= 1;
            girl.upd_temp_stat(STAT_LIBIDO, +20, true);
        }
    }
    ...
    {
        ss << "${name} and the client are bantering like best friends within minutes. She charismatically leads him into conversations where they share interests and deftly steers him away from controversial or uncomfortable subjects. An hour has passed before he even realizes it, and their laughter reverberates around the surrounding tables.\n";
        escort += 2;
        if (girl.has_active_trait("Alcoholic") && rng.percent(50))
        {
            ss << "${name} is laughing a bit louder than he is, though. This is probably because her desperate need for alcohol has resulted in her downing five beverages over the course of the hour. While their conversation was so engaging that the client did not seem to mind how drunk she was getting, he did notice.\n";
            escort -= 1;
    ...
    {
        ss << "${name} has the client laughing at jokes and talking about shared hobbies in no time. The two of them enjoy a pleasant conversation for the next forty-five minutes, and they both share another drink. After about an hour, the conversation starts slowing down as ${name} runs out of ideas to keep it going, but by the end of the evening, she can tell the client enjoyed their time together.\n";
        escort += 1;
        if (girl.has_active_trait("Alcoholic") && rng.percent(50))
        {
            ss << "All except for ${name}'s drinking, that is. As an alcoholic, she is unable to control herself once the drinking starts, and after they share another drink, she gets a third, and then a fourth, and by the fifth she is sloppy. The silent judgment in his eyes verifies that ${name}'s alcoholism has reversed any rapport she was able to gain via conversation.\n";
            escort -= 1;
            girl.upd_temp_stat(STAT_LIBIDO, +20, true);
        }
    }
    ...
    {
        ss << "${name} is able to lead the two of them in a few minutes of good conversation, but it begins to sputter out once she is out of ideas. The client's efforts at leading the conversation would have been successful if ${name} was more charismatic and able to think and speak on her feet. The conversation is not the worst thing in the world, but it ends up being a part of the date that both of them would be happier forgetting.\n";
        if (girl.has_active_trait("Alcoholic") && rng.percent(50))
        {
            ss << "What he cannot forget, however, is how many drinks ${name} knocks back during the hour. Her need is not social; it is clearly an addiction. She is much drunker than he is by the end of the hour, and while he is willing to forget the conversation, he does feel compelled to judge her conduct.\n";
            escort -= 1;
            girl.upd_temp_stat(STAT_LIBIDO, +20, true);
        }
    }
    ...
    if (sex_offer)
    {
        ...
        {
            ss << "${name} is flattered, but not interested in anything with this client. She politely declines his offer, mentioning that her work does not permit it, and lightens the mood with a final few jokes before departing the bar to return to your establishment.";
        }
        else
        {
    ...
    if (sex_event)
    {
        if (titty_lover)
        {
            ...
            ss << "After he erupts cum all over her face, ${name} smiles, cleans herself off, and turns to collect her things to go. \"No need for another drink,\" she grins, \"because I think I just got one. I really do hope you'll call for me again, cutie.\"";
            imagetype = IMGTYPE_TITTY;
            girl.upd_temp_stat(STAT_LIBIDO, -20, true);
        }
        else if (ass_lover)
        ...
        if (!titty_lover && !ass_lover)
        {
            ...
            else if (roll_sex >= 40 || anal)/*Anal*/
            {
                ...
                ss << "${name} lies beside the client for a moment, stroking his chest with her fingers as the cum leaks from her ass. \"That was amazing,\" she purrs, before she stands to gather her clothes to leave. \"I'm glad we got to know each other a little better, cutie, and I really do hope you call for me again sometime soon.\"";
                imagetype = IMGTYPE_ANAL;
                girl.upd_temp_stat(STAT_LIBIDO, -20, true);
            }
            else if (roll_sex >= 25)/*Hand job*/
            ...
            else/*oral*/
            {
                ...
                ss << "${name} licks the remaining cum from her lips with a smile. \"Thanks for the drink,\" she says, as she begins gathering her clothes to leave. \"I had a great time tonight as well, cutie. You should call on me again,\" she suggests as she heads for the door to return to your establishment.";
                imagetype = IMGTYPE_ORAL;
                girl.upd_temp_stat(STAT_LIBIDO, -20, true);
            }
        }
...
break;    // end Es_DeadBeat
    ...
    girl.AddMessage(ss.str(), image_types::work::ESCORT, Day0Night1 ? EVENT_NIGHTSHIFT : EVENT_DAYSHIFT);
